...
export default function App() {
  ...
  async function handleSectionCreate(data) {
    try { await api.createSection(data); await refreshSections(); }
    catch (err) { alert(err.message); }
  }

  async function handleSectionUpdate(id, data) {
    try { await api.updateSection(id, data); await refreshSections(); }
    catch (err) { alert(err.message); }
  }
  ...
  async function handleSectionDelete(id) {
    ...
    try { await api.deleteSection(id); await Promise.all([refreshSections(), refreshTasks()]); }
    catch (err) { alert(err.message); }
  }
  ...
  const completedCount = tasks.filter(t =>
    t.completed && (isManager || String(t.assignee_id) === String(user.id))
  ).length;
  ...
  return (
    <div className="app-shell">
      <Sidebar
        ...
        onOpenContracts={() => setModal({ type: 'contracts' })}
        onLogout={logout}
      />
      ...
      <div className="main-area">
        <div className="main-header">
          ...
          <div className="main-header-right">
            <div className="view-toggle">
              ...
              <button className={`view-btn${view === 'completed' ? ' active' : ''}`} onClick={() => setView('completed')}>
                <i className="ti ti-circle-check" /> Completed{completedCount > 0 && ` (${completedCount})`}
              </button>
              {isManager && (
            ...
            {isManager && view !== 'reports' && view !== 'completed' && (
        ...
        {view !== 'reports' && view !== 'completed' && employees.length > 0 && (
        ...
        {view === 'reports' && isManager && (
          <Reports employees={employees} clients={activeClients} />
        )}
      </div>
